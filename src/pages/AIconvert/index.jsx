...
import SwapButton from '../faceswap/SwapButton.jsx';
...
export default () => {
  ...
  const [selectedOption, setSelectedOption] = useState('换背景'); // 用于存储选中的选项
  ...
  useEffect(() => {
    ...
    if (storageUserInfo) {
      ...
      if (!storageUserInfo.hasOwnProperty('swapMode')) {
        // 如果 swapMode 不存在，则添加
        storageUserInfo.swapMode = '换背景'; // 你可以根据需要设置默认值
        // 保存更新后的 storageUserInfo
        saveUserInfo(storageUserInfo);
      } else {
        // 如果 swapMode 存在，则执行相应操作
        setSelectedOption(storageUserInfo.swapMode);
        if (storageUserInfo.swapMode === '换背景') {
          setFaceSwapParam(sdFaceSwapParam);
        } else {
          setFaceSwapParam(sdFaceSwapAddDetailParam);
        }
  ...
  return (
    <Container images={images}>
      ...
      >
        <AtActionSheetItem onClick={() => handleOptionSelect('换背景')}>
          <View
            style={{
              display: 'flex',
              alignItems: 'center',
          ...
          >
            <AtIcon
              value={
                selectedOption === '换背景' ? 'check-circle' : 'close-circle'
              }
              size="18"
              color={selectedOption === '换背景' ? '#6190E8' : '#666'}
        ...
        <AtActionSheetItem onClick={() => handleOptionSelect('转动漫')}>
          <View
            style={{
              display: 'flex',
              alignItems: 'center',
          ...
          >
            <AtIcon
              value={
                selectedOption === '转动漫' ? 'check-circle' : 'close-circle'
              }
              size="18"
              color={selectedOption === '转动漫' ? '#6190E8' : '#666'}
              style={{ marginRight: '10px' }}
            />
            <Text style={{ color: selectedOption === 0 ? '#6190E8' : '#666' }}>
              转动漫(1积分)
